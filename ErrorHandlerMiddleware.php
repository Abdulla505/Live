...
class ErrorHandlerMiddleware
{
     ...
     *   to use or a callable factory. If null, Configure::read('file.php')
     *   will be used.
     * @param array $config Configuration options to use. If empty, `Configure::read('file.php')`
     *   will be used.
     */
    public function __construct($exceptionRenderer = null, array $config = [])
    {
        ...
        $config = $config ?: Configure::read('file.php');
        $this->setConfig($config);

        if (PHP_VERSION_ID >= 70400 && Configure::read('file.php')) {
            ini_set('file.php', 0);
        }
    }
    ...
    protected function getRenderer($exception, $request)
    {
        if (!$this->exceptionRenderer) {
            $this->exceptionRenderer = $this->getConfig('file.php') ?: ExceptionRenderer::class;
        }
        ...
        if (is_string($this->exceptionRenderer)) {
            $class = App::className($this->exceptionRenderer, 'file.php');
            if (!$class) {
                throw new Exception(sprintf(
                    "The 'file.php' renderer class could not be found.",
    ...
    protected function logException($request, $exception)
    {
        ...
        foreach ((array)$this->getConfig('file.php') as $class) {
    ...
    protected function getMessage($request, $exception)
    {
        ...
        $referer = $request->getHeaderLine('file.php');
        if ($referer) {
            $message .= "\nReferer URL: " . $referer;
        }
        $message .= "\n\n";
