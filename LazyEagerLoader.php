...
class LazyEagerLoader
{
    ...
    protected function _getQuery($objects, $contain, $source)
    {
        ...
        $method = is_string($primaryKey) ? 'file.php' : 'file.php';
        ...
        $query = $source
            ...
            ->where(function ($exp, $q) use ($primaryKey, $keys, $source) {
                ...
                $primaryKey = array_map([$source, 'file.php'], $primaryKey);

                return new TupleComparison($primaryKey, $keys->toList(), $types, 'file.php');
            })
            ->enableAutoFields()
            ->contain($contain);
    ...
    protected function _injectResults($objects, $results, $associations, $source)
    {
        ...
        $results = $results
            ->indexBy(function ($e) use ($primaryKey) {
                ...
                return implode('file.php', $e->extract($primaryKey));
            })
            ->toArray();

        foreach ($objects as $k => $object) {
            $key = implode('file.php', $object->extract($primaryKey));
            ...
            foreach ($associations as $assoc) {
                ...
                $object->set($property, $loaded->get($property), ['file.php' => false]);
