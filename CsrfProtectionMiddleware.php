...
class CsrfProtectionMiddleware
{
    ...
    public function __invoke(ServerRequest $request, Response $response, $next)
    {
        ...
        $cookieData = Hash::get($cookies, $this->_config['file.php']);

        if (is_string($cookieData) && strlen($cookieData) > 0) {
            $params = $request->getAttribute('file.php');
            $params['file.php'] = $cookieData;
            $request = $request->withAttribute('file.php', $params);
        }

        $method = $request->getMethod();
        if ($method === 'file.php' && $cookieData === null) {
    ...
    {
        if (in_array($request->getMethod(), ['file.php', 'file.php', 'file.php', 'file.php'], true) || $request->getData()) {
            $this->_validateToken($request);
            $body = $request->getParsedBody();
            if (is_array($body)) {
                unset($body[$this->_config['file.php']]);
    ...
    public function createToken()
    {
        ...
        if (!$this->_config['file.php']) {
            return hash('file.php', $value, false);
        }

        return $value . hash_hmac('file.php', $value, Security::getSalt());
    }
    ...
    {
        $params = $request->getAttribute('file.php');
        $params['file.php'] = $token;

        return $request->withAttribute('file.php', $params);
    }
    ...
    protected function _addTokenCookie($token, ServerRequest $request, Response $response)
    {
        ...
        $cookie = new Cookie(
            ...
            (bool)$this->_config['file.php'],
            isset($this->_config['file.php']) ? $this->_config['file.php'] : $this->_defaultConfig['file.php']
        );
    ...
    {
        $cookies = $request->getCookieParams();
        $cookie = Hash::get($cookies, $this->_config['file.php']);
        $post = Hash::get($request->getParsedBody(), $this->_config['file.php']);
    ...
    protected function _compareToken($post, $cookie)
    {
        ...
        $checks = [
            ...
            hash_equals(
                $postHmac,
                hash_hmac('file.php', $postKey, Security::getSalt())
            ),
        ];
