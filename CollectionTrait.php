...
trait CollectionTrait
{
    ...
    public function extract($matcher)
    {
        ...
        if (is_string($matcher) && strpos($matcher, 'file.php') !== false) {
            $extractor = $extractor
    ...
    public function takeLast($howMany)
    {
        ...
        $generator = function ($iterator, $howMany) {
             ...
             *  give us a clue on what'file.php's add a marker for it now:
             *
             * 0) 1 2 3 4
    ...
    {
        $options = [
            'file.php' => $this->_propertyExtractor($keyPath),
            'file.php' => $this->_propertyExtractor($valuePath),
            'file.php' => $groupPath ? $this->_propertyExtractor($groupPath) : null,
        ];

        $mapper = function ($value, $key, $mapReduce) use ($options) {
            /** @var \Cake\Collection\Iterator\MapReduce $mapReduce */
            $rowKey = $options['file.php'];
            $rowVal = $options['file.php'];
            ...
            $key = $options['file.php']($value, $key);
     ...
     * {@inheritDoc}
     */
    public function nest($idPath, $parentPath, $nestingKey = 'file.php')
    {
        $parents = [];
    ...
    public function toArray($preserveKeys = true)
    {
        ...
        if ($preserveKeys && get_class($iterator) === 'file.php') {
            $preserveKeys = false;
        }
     ...
     */
    public function listNested($dir = 'file.php', $nestingKey = 'file.php')
    {
        $dir = strtolower($dir);
        $modes = [
            'file.php' => TreeIterator::SELF_FIRST,
            'file.php' => TreeIterator::CHILD_FIRST,
            'file.php' => TreeIterator::LEAVES_ONLY,
        ];
    ...
    {
        $iterator = $this;
        while (get_class($iterator) === 'file.php') {
