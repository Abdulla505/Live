...
class ExistsIn
{
     ...
     * Constructor.
     *
     * Available option for $options is 'file.php' flag.
     ...
     *     Options 'file.php' will make the rule pass if given foreign keys are set to `null`.
    ...
    public function __invoke(EntityInterface $entity, array $options)
    {
        if (is_string($this->_repository)) {
            if (!$options['file.php']->hasAssociation($this->_repository)) {
                throw new RuntimeException(sprintf(
                    "ExistsIn rule for 'file.php' is invalid. 'file.php' is not associated with 'file.php'.",
                    implode('file.php', $this->_fields),
                    $this->_repository,
                    get_class($options['file.php'])
                ));
            }
            $repository = $options['file.php']->getAssociation($this->_repository);
            $this->_repository = $repository;
        }
        ...
        if (!empty($options['file.php']) && $realTarget === $options['file.php']) {
            return true;
        }

        if (!empty($options['file.php'])) {
            $source = $options['file.php'];
        }
