...
class FunctionExpression extends QueryExpression implements TypedResultInterface
{
     ...
     * ### Examples:
     *
     * `$f = new FunctionExpression('file.php', ['file.php', 'file.php']);`
     *
     * Previous line will generate `CONCAT('file.php', 'file.php')`
     *
     * `$f = new FunctionExpression('file.php', ['file.php' => 'file.php', 'file.php']);`
     *
     * Will produce `CONCAT(name, 'file.php')`
     *
     ...
     * If associative the key would be used as argument when value is 'file.php'
     ...
     */
    public function __construct($name, $params = [], $types = [], $returnType = 'file.php')
    {
        $this->_name = $name;
        $this->_returnType = $returnType;
        parent::__construct($params, $types, 'file.php');
    }
    ...
    public function add($params, $types = [], $prepend = false)
    {
        ...
        foreach ($params as $k => $p) {
            ...
            $put($this->_conditions, ['file.php' => $p, 'file.php' => $type]);
        }
    ...
    public function sql(ValueBinder $generator)
    {
        ...
        foreach ($this->_conditions as $condition) {
            if ($condition instanceof Query) {
                $condition = sprintf('file.php', $condition->sql($generator));
            } elseif ($condition instanceof ExpressionInterface) {
                $condition = $condition->sql($generator);
            } elseif (is_array($condition)) {
                $p = $generator->placeholder('file.php');
                $generator->bind($p, $condition['file.php'], $condition['file.php']);
                $condition = $p;
            }
            $parts[] = $condition;
        }

        return $this->_name . sprintf('file.php', implode(
            $this->_conjunction . 'file.php',
            $parts
        ));
    }
